...
const StatCard = ({ icon: Icon, label, value, sub, color = 'text-white', bg = 'from-white/5 to-white/[0.02]' }) => (
      ...
      <aside className="hidden md:flex flex-col w-64 bg-[#0a0c16] border-r border-white/8 h-screen sticky top-0 shrink-0">
        ...
        <div className="px-6 py-6 border-b border-white/8">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-white font-black text-base">Provider Hub</span>
          </div>
        </div>
        ...
        <div className="px-5 py-4 border-b border-white/8">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-600 flex items-center justify-center text-white font-black text-sm shadow-md shrink-0">
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="md:hidden flex items-center justify-between px-5 py-4 border-b border-white/8 bg-[#0a0c16]/80 backdrop-blur-xl sticky top-0 z-30">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-black">Provider Hub</span>
          </div>
          <button onClick={handleLogout} className="p-2 rounded-xl text-zinc-400 hover:text-red-400 hover:bg-red-500/10 cursor-pointer">
            <LogOut size={16} />
          </button>
        </div>
        {/* Mobile nav tabs */}
        <div className="md:hidden flex gap-1 px-4 py-3 border-b border-white/8 overflow-x-auto">
          {navItems.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setTab(id)} className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all cursor-pointer border ${
              tab === id ? 'bg-amber-500/15 text-amber-400 border-amber-500/25' : 'text-zinc-400 border-transparent hover:bg-white/5'
            }`}>
              <Icon size={13} />{label}
            </button>
          ))}
        ...
        <div className="px-5 md:px-8 py-7 space-y-8">
          ...
          {tab === 'dashboard' && (
            <>
              <div>
                <h1 className="text-2xl font-black text-white">Welcome back, {profile?.full_name?.split(' ')[0] || 'Provider'} 👋</h1>
                <p className="text-zinc-500 text-sm mt-1">Here's what's happening with your bookings today.</p>
              </div>

              {/* Stats */}
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard icon={BookOpen}    label="Total Jobs"  value={total}     color="text-white" />
              ...
              {profile?.average_rating != null && (
                <div className="flex items-center gap-4 p-5 bg-gradient-to-r from-amber-500/10 to-orange-500/5 border border-amber-500/20 rounded-2xl">
                  ...
                  <div>
                    <p className="text-white font-bold">Your Rating</p>
                    <p className="text-zinc-500 text-xs">{profile.total_reviews || 0} review{profile.total_reviews !== 1 ? 's' : ''} received</p>
                  </div>
                  <div className="ml-auto text-right">
                    <p className="text-zinc-500 text-xs">{profile.experience_years || 0} yrs experience</p>
                    <p className="text-zinc-500 text-xs">{profile.city || '—'}</p>
                  </div>
                </div>
              )}
          ...
          {tab === 'bookings' && (
            <>
              <div>
                ...
                <p className="text-zinc-500 text-sm mt-1">Manage and track all your service bookings.</p>
              </div>
            ...
            <>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-zinc-500 text-sm mt-1">Manage your provider information and service details.</p>
                </div>
                {!editMode && (
                  <button
              ...
              <div className="grid md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-1 space-y-4">
                  <div className="bg-[#0d1020] border border-white/8 rounded-2xl p-6 text-center">
                    <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-amber-400 to-orange-600 flex items-center justify-center text-white font-black text-3xl shadow-xl mx-auto">
                      {(profile?.full_name || '?')[0].toUpperCase()}
                    </div>
                ...
                <div className="md:col-span-2">
                    ...
                    <form onSubmit={handleSaveProfile} className="bg-[#0d1020] border border-white/8 rounded-2xl p-6 space-y-5">
                        ...
                        {[
                          ...
                          <div key={key}>
                            ...
                            <input
                              ...
                              className="w-full bg-white/5 border border-white/10 rounded-xl text-white text-sm px-3 py-2.5 focus:border-amber-500/50 outline-none transition-all"
                            />
                          </div>
                        ))}
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-white/5 border border-white/10 rounded-xl text-white text-sm px-3 py-2.5 focus:border-amber-500/50 outline-none"
                        />
                      </div>
                      <div>
                        <label className="block text-zinc-500 text-xs font-bold mb-1.5">About / Description</label>
                        <textarea
                          value={editForm.description || ''}
                          onChange={e => setEditForm({ ...editForm, description: e.target.value })}
                          rows={3}
                          className="w-full bg-white/5 border border-white/10 rounded-xl text-white text-sm px-3 py-2.5 focus:border-amber-500/50 outline-none resize-none"
                        />
                      </div>
                    ...
                    <div className="bg-[#0d1020] border border-white/8 rounded-2xl p-6 space-y-5">
                      ...
                      {profile?.description && (
                        <div className="pt-2 border-t border-white/5">
                          <p className="text-zinc-500 text-xs font-bold uppercase tracking-wider mb-2">About</p>
                          <p className="text-white text-sm leading-relaxed">{profile.description}</p>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {profile?.documents?.length > 0 && (
                    <div className="bg-[#0d1020] border border-white/8 rounded-2xl p-6 mt-4">
                      <h3 className="text-white font-bold mb-4">Documents</h3>
                      <div className="space-y-3">
                        {profile.documents.map(doc => (
                          <div key={doc.document_id} className="flex items-center justify-between bg-white/[0.025] border border-white/5 rounded-xl px-4 py-3">
                            <div>
                              <p className="text-white text-sm font-semibold capitalize">{doc.document_type?.replace(/_/g, ' ')}</p>
                              <p className="text-zinc-500 text-xs">Uploaded {doc.uploaded_at ? new Date(doc.uploaded_at).toLocaleDateString('en-IN') : '—'}</p>
                            </div>
                            <div className="flex items-center gap-2">
    ...
    <div className="bg-[#0d1020] border border-white/8 rounded-2xl overflow-hidden">
      {fullView && (
        <div className="px-5 py-4 border-b border-white/8 flex flex-col sm:flex-row gap-3">
          <input
            ...
            className="bg-white/5 border border-white/10 text-white text-xs px-3 py-2 rounded-xl outline-none focus:border-amber-500/40 placeholder-zinc-500 w-full sm:w-64"
          />
          <div className="flex gap-1.5 flex-wrap">
            {statuses.map(s => (
              <button key={s} onClick={() => setFilter(s)} className={`px-2.5 py-1 rounded-lg text-[10px] font-bold cursor-pointer border transition-all ${filter===s?'bg-amber-500/15 text-amber-400 border-amber-500/25':'text-zinc-500 border-white/5 hover:text-white hover:bg-white/5'}`}>{s}</button>
            ))}
          </div>
        </div>
      )}
        ...
        <table className="w-full text-sm text-left">
          <thead>
            <tr className="border-b border-white/5 bg-white/[0.015]">
              ...
              <th className="px-4 py-3 text-zinc-500 text-xs font-bold uppercase">Customer</th>
              ...
              <th className="px-4 py-3 text-zinc-500 text-xs font-bold uppercase hidden sm:table-cell">Price</th>
            ...
            {(fullView ? filtered : filtered.slice(0,8)).map(b => (
              <tr key={b.booking_id} className="border-b border-white/5 hover:bg-white/[0.015] transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-[10px] font-black shrink-0">
                      {(b.customer_name||'?')[0].toUpperCase()}
                    </div>
                    <span className="text-white text-xs font-semibold">{b.customer_name}</span>
                  </div>
                </td>
      ...
      {!fullView && filtered.length > 8 && (
        <div className="px-4 py-3 border-t border-white/5 text-center">
          <p className="text-zinc-500 text-xs">{filtered.length - 8} more bookings — switch to Bookings tab to see all</p>
        </div>
      )}
    </div>
  );
