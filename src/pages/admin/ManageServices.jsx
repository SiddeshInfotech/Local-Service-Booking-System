  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">Manage Services</h1>
          <p className="text-[var(--color-text-secondary)] text-sm mt-1">Add, edit or remove individual services.</p>
        </div>
        <button
          onClick={openAdd}
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-[var(--color-text-primary)] text-sm font-semibold transition-colors cursor-pointer"
        >
      ...
      <div className="rounded-2xl bg-[var(--color-card-bg)] border border-[var(--color-border-subtle)] overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--color-border-subtle)] bg-[var(--color-secondary-bg)]">
                <th className="text-left px-5 py-3.5 text-[var(--color-text-secondary)] text-xs font-semibold uppercase tracking-wide">Service</th>
                <th className="text-left px-5 py-3.5 text-[var(--color-text-secondary)] text-xs font-semibold uppercase tracking-wide hidden md:table-cell">Category</th>
                <th className="text-left px-5 py-3.5 text-[var(--color-text-secondary)] text-xs font-semibold uppercase tracking-wide">Price</th>
                <th className="text-left px-5 py-3.5 text-[var(--color-text-secondary)] text-xs font-semibold uppercase tracking-wide hidden lg:table-cell">Duration</th>
                <th className="text-left px-5 py-3.5 text-[var(--color-text-secondary)] text-xs font-semibold uppercase tracking-wide">Status</th>
                <th className="text-left px-5 py-3.5 text-[var(--color-text-secondary)] text-xs font-semibold uppercase tracking-wide">Actions</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="text-center text-[var(--color-text-secondary)] py-10">No services found.</td></tr>
              ) : (
                filtered.map((s) => (
                  <tr key={s.id} className="border-b border-[var(--color-border-subtle)] hover:bg-[var(--color-secondary-bg)] transition-colors">
                    <td className="px-5 py-4 text-[var(--color-text-primary)] text-xs font-medium">{s.name}</td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-0.5 text-[var(--color-text-primary)] text-xs font-semibold">
                        <IndianRupee size={11} />
                        {Number(s.price).toLocaleString()}
                      </div>
                    </td>
                    <td className="px-5 py-4 text-[var(--color-text-secondary)] text-xs hidden lg:table-cell">{s.duration}</td>
                    <td className="px-5 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-semibold border ${
                        s.status === 'Active'
                          ? 'bg-green-500/15 text-green-400 border-green-500/20'
                          : 'bg-[var(--color-zinc-700)]/30 text-[var(--color-text-secondary)] border-[var(--color-border-subtle)]'
                      }`}>
                        {s.status}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEdit(s)} className="p-1.5 rounded-lg text-[var(--color-text-secondary)] hover:text-amber-400 hover:bg-amber-500/10 transition-colors cursor-pointer">
                          <Pencil size={14} />
                        </button>
                        <button onClick={() => handleDelete(s.id)} className="p-1.5 rounded-lg text-[var(--color-text-secondary)] hover:text-red-400 hover:bg-red-500/10 transition-colors cursor-pointer">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
        ...
        <div className="px-5 py-3 border-t border-[var(--color-border-subtle)]">
          <p className="text-[var(--color-text-secondary)] text-xs">Showing {filtered.length} of {services.length} services</p>
        </div>
      </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-[var(--color-card-bg)] border border-[var(--color-border-subtle)] rounded-2xl p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-[var(--color-text-primary)] font-bold text-base">{modal === 'add' ? 'Add New Service' : 'Edit Service'}</h3>
              <button onClick={closeModal} className="p-1.5 rounded-lg text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-overlay-subtle)] transition-colors cursor-pointer">
                <X size={18} />
              </button>
            </div>
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-[var(--color-overlay-subtle)] border border-[var(--color-border-subtle)] text-[var(--color-text-primary)] text-sm px-4 py-2.5 rounded-xl outline-none focus:border-blue-500 transition-colors"
                >
