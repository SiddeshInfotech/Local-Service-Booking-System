...
const ProviderApproval = () => {
  ...
  const tabStyle = (key, color) => {
    if (tab !== key) return 'text-[var(--color-text-secondary)] border-transparent hover:text-[var(--color-text-primary)] hover:bg-[var(--color-overlay-subtle)]';
    return {
    ...
    <div className="space-y-6 text-left relative z-10 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[var(--color-text-secondary)] text-sm mt-1">Audit verification documents, business certifications, and moderate platform signup requests.</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 border-b border-[var(--color-border-subtle)] pb-0">
        {tabs.map(({ key, label, color }) => (
          ...
          >
            {label}
            <span className="px-2 py-0.5 rounded-full text-[9px] bg-[var(--color-overlay-subtle)] border border-[var(--color-border-subtle)] text-[var(--color-text-secondary)] font-black">
      ...
      {/* Cards */}
      {loading ? (
        <div className="text-center py-20 rounded-3xl bg-[var(--color-card-bg)] border border-[var(--color-border-subtle)] backdrop-blur-xl flex justify-center">
          <Loader2 size={32} className="animate-spin text-blue-400" />
        </div>
      ) : current.length === 0 ? (
        <div className="text-center py-20 rounded-3xl bg-[var(--color-card-bg)] border border-[var(--color-border-subtle)] backdrop-blur-xl">
          <ShieldCheck size={40} className="mx-auto mb-4 text-zinc-700" />
          <p className="text-[var(--color-text-secondary)] text-sm font-semibold">No provider accounts in this list.</p>
            ...
            <div key={p.id} className="rounded-3xl bg-[var(--color-card-bg)] border border-[var(--color-border-subtle)] hover:border-[var(--color-border-subtle)] p-6 flex flex-col justify-between shadow-lg hover:shadow-2xl hover:scale-[1.01] transition-all duration-300">
              ...
              <div>
                ...
                <div className="flex items-start gap-3.5">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-500 to-blue-600 flex items-center justify-center text-[var(--color-text-primary)] font-black text-sm flex-shrink-0 shadow-md">
                    {p.name[0]?.toUpperCase()}
                  </div>
                  <div className="min-w-0 flex-1 text-left">
                    <p className="text-[var(--color-text-primary)] font-bold text-xs truncate leading-normal">{p.name}</p>
                    <p className="text-[var(--color-text-secondary)] text-[10px] truncate mt-0.5">{p.email}</p>
                  </div>
                ...
                {/* Details */}
                <div className="grid grid-cols-2 gap-4 mt-6 border-t border-b border-[var(--color-border-subtle)] py-4 my-4">
                  ...
                  {p.documents && p.documents.length > 0 && (
                    <div className="flex items-center gap-2 text-xs text-[var(--color-text-secondary)] col-span-2">
                      ...
                      >
                        <span>{p.docType}</span>
                        <Eye size={12} className="text-[var(--color-text-secondary)]" />
                      </button>
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between text-xs text-[var(--color-text-secondary)] px-1">
                  <span>Experience: <strong className="text-[var(--color-text-primary)] font-bold">{p.experience}</strong></span>
                  {p.rating != null && (
                    <div className="flex items-center gap-1">
                      <Star size={11} className="text-amber-400 fill-amber-400" />
                      <span className="text-[var(--color-text-primary)] font-bold">{Number(p.rating).toFixed(1)}</span>
                    </div>
                  )}
                </div>
              </div>
      ...
      {selectedDoc && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fade-in text-left">
          <div className="relative w-full max-w-lg bg-[var(--color-card-bg)] border border-[var(--color-border-subtle)] rounded-[32px] p-6 shadow-2xl">
            ...
            <h3 className="text-[var(--color-text-primary)] font-bold text-lg mb-1">Document Audit Panel</h3>
            <p className="text-[var(--color-text-secondary)] text-xs mb-5">Inspect license certificates uploaded by <span className="text-[var(--color-text-primary)] font-bold">{selectedDoc.name}</span></p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fade-in text-left">
          <div className="relative w-full max-w-sm bg-[var(--color-card-bg)] border border-[var(--color-border-subtle)] rounded-[32px] p-6 shadow-2xl">
            ...
            <p className="text-[var(--color-text-secondary)] text-xs mt-2 leading-relaxed">
              Are you sure you want to change status for <span className="text-[var(--color-text-primary)] font-bold">{showConfirmAction.provider.name}</span> to <span className="text-blue-400 font-bold">{showConfirmAction.to}</span>?
            </p>
            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setShowConfirmAction(null)} className="px-4 py-2 border border-[var(--color-border-subtle)] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] rounded-xl text-xs cursor-pointer">Cancel</button>
              <button 
                ...
                className={`px-5 py-2.5 font-bold rounded-xl text-xs cursor-pointer shadow-lg flex items-center gap-2 ${
                  showConfirmAction.to === 'Approved' 
                    ? 'bg-green-600 hover:bg-green-500 text-[var(--color-text-primary)] shadow-green-500/10' 
                    : 'bg-red-600 hover:bg-red-500 text-[var(--color-text-primary)] shadow-red-500/10'
                }`}
              >
